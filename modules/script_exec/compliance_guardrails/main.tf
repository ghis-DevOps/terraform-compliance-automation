# AWS Config needs an IAM role, delivery channel, and active recorder before
# managed rules can be created.
...
resource "aws_iam_role" "config" {
  name               = "${var.environment}-config-recorder-role"
...
resource "aws_s3_bucket" "config" {
  bucket_prefix = "${var.environment}-config-"
}
...
# AWS Config Rule: Ensure S3 buckets are encrypted
resource "aws_config_config_rule" "s3_bucket_server_side_encryption_enabled" {
  name        = "${var.environment}-s3-bucket-encryption-check"
...
# AWS Config Rule: Ensure required tags are present on resources
resource "aws_config_config_rule" "required_tags_check" {
  name        = "${var.environment}-required-tags-check"
